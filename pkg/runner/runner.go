...
func (r *Runner) onReceive(hostResult *result.HostResult) {
	...
	// Grab banner immediately if requested (only for TCP ports)
	if r.options.BannerGrab && !r.options.Stream && !r.options.Passive {
		for _, p := range hostResult.Ports {
			if p.Banner == "" {
				p.Banner = grabProtocolBanner(hostResult.IP, p.Port, p.Protocol, r.options.GetTimeout())
			}
		}
	}
...
func (r *Runner) handleHostPort(ctx context.Context, host, payload string, p *port.Port) {
	...
	default:
		...
		if open && err == nil {
			if r.options.BannerGrab && p.Protocol == protocol.TCP {
				// We need the actual connection from ConnectPort – see enhancement below
			}
...
func (r *Runner) grabBannersForScanResults() {
	bannerTimeout := r.options.GetTimeout()

	for hostResult := range r.scanner.ScanResults.GetIPsPorts() {
		for _, p := range hostResult.Ports {
			// Only grab a banner if one hasn’t already been obtained
			if p.Banner == "" {
				banner := grabProtocolBanner(hostResult.IP, p.Port, p.Protocol, bannerTimeout)
				if banner != "" {
					p.Banner = banner
				}
			}
